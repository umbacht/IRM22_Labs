...
disp("The brief impulse sets the ball in motion. After that impulse, the plate is in its original (non tilted) position again,"+newline+ ...
    "however, the ball keeps on rolling given the initial impulse for movement. "+newline+ ...
    "As the system will not converge to 0, it is unstable.")
disp("Step response: response of the OL system as reaction to a step signal on the input.")
disp("By setting a step input, the system reacts by tilting the plate to move the ball. "+newline+ ...
    "Without feedback however, the system will not track the position of the ball and just remain in its tilted position. "+newline+ ...
...
disp("Prelab Q2")
disp("The response for the case Kd=2 is underdamped. By increasing Kd, the system is damped "+newline+ ...
    "more heavily and reacts slower to the input. ")
disp("Considering the real system, by increasing the gain Kd the system could become too slow. "+newline+...
    "As the object tracking by the pixy cam also introduces a delay in the system, it might be too slow " +newline+...
...
disp("Prelab Q3")
disp("The system will become more unstable and therefore more difficult to control with " + newline+...
    "higher time delays. The position of the ball is measured at a time t, processed, and fed " +newline+...
    "to the controller with a delay dt. This means that the system at time t will base its control on" +newline+...
    "the position of the ball at time t-dt instead of t. Meaning that the system will most likely assume an " +newline+...
...
disp("Prelab Q4")
disp("The motor saturation can lead to an integrator wind-up. By having too high a set point" +newline+...
    "that the system can physically not follow, the integral error term will increase. Due to that" +newline+...
